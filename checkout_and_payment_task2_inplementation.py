...
def check_cart(user, cart):
    ...
    if question.lower() == "y":
        payment_choice = input("\nDo you want to pay with your wallet or a card? (wallet/card): ")

        if payment_choice.lower() == "wallet":
            # Perform wallet payment
            checkout(user, cart)
        elif payment_choice.lower() == "card":
            # Display the list of cards
            print("\nYour cards:")
            for i, card in enumerate(user.cards):
                print(f"{i + 1}. {card}")

            # Ask which card the user wants to use
            card_choice = input("Enter the number of the card you want to use: ")
            if card_choice.isdigit() and 1 <= int(card_choice) <= len(user.cards):
                # Perform card payment
                print(f"Payment using card {card_choice} successful.")
                for item in cart.items:
                    item.units -= 1
                    if item.units == 0:
                        products.remove(item)
                ...
                print("\nInvalid card choice. Payment failed.")
        else:
...
def checkoutAndPayment(login_info):
    ...
    while True:
        ...
        # Ask the user whether to use wallet or card
